...
def objective(trial):
    ...
    if trial.suggest_categorical("use_lr", [True, False]):
        add_model("lr", LogisticRegression(C=trial.suggest_float("lr_C", 0.01, 1.0), solver="liblinear", class_weight="balanced", max_iter=300), "lr_w")
    if trial.suggest_categorical("use_xgb", [True, False]):
        add_model("xgb", XGBClassifier(n_estimators=trial.suggest_int("xgb_n", 100, 300), max_depth=trial.suggest_int("xgb_d", 3, 10), learning_rate=trial.suggest_float("xgb_lr", 0.01, 0.2), subsample=trial.suggest_float("xgb_sub", 0.5, 1.0), colsample_bytree=trial.suggest_float("xgb_col", 0.5, 1.0), use_label_encoder=False, eval_metric="logloss", verbosity=0), "xgb_w")
    if trial.suggest_categorical("use_lgbm", [True, False]):
        add_model("lgbm", LGBMClassifier(n_estimators=trial.suggest_int("lgbm_n", 100, 300), max_depth=trial.suggest_int("lgbm_d", 3, 10), learning_rate=trial.suggest_float("lgbm_lr", 0.01, 0.2), verbose=-1), "lgbm_w")
    if trial.suggest_categorical("use_cat", [True, False]):
        add_model("cat", CatBoostClassifier(iterations=trial.suggest_int("cat_n", 100, 300), depth=trial.suggest_int("cat_d", 3, 10), learning_rate=trial.suggest_float("cat_lr", 0.01, 0.2), verbose=0), "cat_w")
    if trial.suggest_categorical("use_mlp", [True, False]):
        add_model("mlp", MLPClassifier(hidden_layer_sizes=(trial.suggest_int("mlp_l1", 50, 200), trial.suggest_int("mlp_l2", 20, 100)), alpha=trial.suggest_float("mlp_alpha", 1e-5, 1e-2), max_iter=500), "mlp_w")
    if trial.suggest_categorical("use_svc", [True, False]):
        add_model("svc", SVC(C=trial.suggest_float("svc_C", 0.1, 10.0), probability=True, class_weight="balanced"), "svc_w")
    if trial.suggest_categorical("use_knn", [True, False]):
        add_model("knn", KNeighborsClassifier(n_neighbors=trial.suggest_int("knn_k", 3, 10)), "knn_w")
    if trial.suggest_categorical("use_et", [True, False]):
        add_model("et", ExtraTreesClassifier(n_estimators=trial.suggest_int("et_n", 100, 300), max_depth=trial.suggest_int("et_d", 5, 30)), "et_w")
    if trial.suggest_categorical("use_gb", [True, False]):
        add_model("gb", GradientBoostingClassifier(n_estimators=trial.suggest_int("gb_n", 100, 300), max_depth=trial.suggest_int("gb_d", 3, 10), learning_rate=trial.suggest_float("gb_lr", 0.01, 0.2)), "gb_w")
    if trial.suggest_categorical("use_hist", [True, False]):
        add_model("hist", HistGradientBoostingClassifier(max_iter=trial.suggest_int("hist_iter", 100, 300)), "hist_w")
    if trial.suggest_categorical("use_bag", [True, False]):
        add_model("bag", BaggingClassifier(n_estimators=trial.suggest_int("bag_n", 10, 50)), "bag_w")
    if trial.suggest_categorical("use_ridge", [True, False]):
        add_model("ridge", RidgeClassifier(alpha=trial.suggest_float("ridge_alpha", 0.1, 5.0), class_weight="balanced"), "ridge_w")
    if trial.suggest_categorical("use_dt", [True, False]):
        add_model("dt", DecisionTreeClassifier(max_depth=trial.suggest_int("dt_d", 3, 20), class_weight="balanced"), "dt_w")
    if trial.suggest_categorical("use_nb", [True, False]):
        add_model("nb", GaussianNB(), "nb_w")
